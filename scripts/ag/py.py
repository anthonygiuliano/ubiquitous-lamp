...
sf_stdout: subprocess.CompletedProcess[bytes] = subprocess.run(args=['sf','org','display','user','--json'], stdout=subprocess.PIPE)
...
sf: Salesforce = Salesforce(instance_url=sf_dict['instanceUrl'],session_id=sf_dict['accessToken'])

# Get users from target org
users = sf.query(query='SELECT Id, FirstName, LastName, Email, Username FROM User')['records']
...
users_df[['Username_Name', 'Username_Domain']] = users_df['Username'].str.split(pat='@', expand=True)
...
metadata_dir: str = os.path.abspath(path=os.path.join(script_dir, '..', '..', 'force-app'))
...
for xml_path in xml_paths:
	...
	for elem in root.iter():
		...
		if match:
			...
			username_new: str = users_df.loc[users_df['Username_Name'] == username_name, 'Username'].values[0]
			email_new: str = users_df.loc[users_df['Username_Name'] == username_name, 'Email'].values[0]
