...
class SfCliHelper:
    ...
    def get_fixes(cls, row: pd.Series) -> pd.Series:
        error: str = row["error"]
        for enum in SfErrors:
            match: re.Match[str] | None = re.search(
                pattern=enum.value["ID"].value, string=error
            )
            if match:
                row["sfError"] = enum
                row["fixPattern"] = enum.value["FIX"].value
                row["fixFunction"] = enum.value["FN"]
                row["idMatchGroups"] = match.groupdict()
                break
        return row
    ...
    def process_dr(cls) -> None:
        for index, row in cls.DR_DF.iterrows():
            file_path: str = row["filePath"] if "filePath" in row else ""
            fix_pattern: re.Pattern[str] = row["fixPattern"]
            fix_function: Callable[[str, re.Pattern, pd.DataFrame], None] = row[
                "fixFunction"
            ]
            fix_function(file_path, fix_pattern, cls.USERS_DF)
        return None
